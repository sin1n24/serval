...
- **呼出**ボタン（[line-call-system](https://github.com/sin1n24/line-call-system) 連携）— 選手の「番号」(=チームID, 数字)で LINE 待機通知を送信。結果入力パネルの「対戦チームを呼出」で両チームを一括呼出
- **スポンサー広告**（左カラム「広告設定」）— 画像3枠をブラケット上に配置（管理画面はDnDで移動・リンクへ飛ばない／閲覧画面はクリックでリンクを開く）。全体閲覧数（表示回数）と各リンクのクリック数を集計（state本体とは別レコードに保存）
